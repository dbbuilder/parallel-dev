...
CREATE TABLE IF NOT EXISTS requirements (
    ...
    priority TEXT CHECK(priority IN ('must', 'should', 'could', 'wont')) DEFAULT 'should',
    status TEXT CHECK(status IN ('not_implemented', 'in_progress', 'implemented', 'verified')) DEFAULT 'not_implemented',
...
SELECT 
    ...
    COUNT(DISTINCT CASE WHEN r.status = 'implemented' THEN r.id END) as implemented_requirements,
    CASE 
        WHEN COUNT(DISTINCT t.id) > 0 
        THEN CAST(COUNT(DISTINCT CASE WHEN t.status = 'completed' THEN t.id END) AS REAL) / COUNT(DISTINCT t.id) * 100
        ELSE 0 
